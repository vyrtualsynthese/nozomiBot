...
module.exports = class TwitchWebhook {
     ...
     * @param {TwitchAPIHandler} twitchApiHandler
     * @param {ParametersRepository} parametersRepo
     */
    constructor (logger, server, cacheManager, streamInfoRepository, twitchConnector, twitchApiHandler, parametersRepo) {
    ...
    async _handleFollowsWebhook (url, request, response, body) {
        ...
        if (body.data && body.data.length > 0) {
            ...
            const [ user, parameters ] = await Promise.all([
                this.twitchApiHandler.getUserInfo(userId),
                this.parametersRepo.retrieve(),
            ]);

            if (!parameters.displayFollows) {
                return response.end();
            }

            // TODO : register in database

            const message = parameters.followsMessage.replace(/%username%/, user.display_name);
