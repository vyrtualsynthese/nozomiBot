...
const TwitchUserProvider = require('../Provider/TwitchUserProvider');
const tmi = require('tmi.js');
const moment = require('moment');

module.exports = class TwitchConnectorIO extends ConnectorIO {
    constructor (logger, userRepository, twitchApiHandler) {
        super();

        this.client = null;
        this.logger = logger.child({ subject: this.constructor.name });
        this.userRepository = userRepository;
        this.userProvider = new TwitchUserProvider(logger, twitchApiHandler);
    }
    ...
    onMessage (channel, userstate, message) {
        ...
        switch (userstate['message-type']) {
            case 'action':
                break;
            case 'chat': {
                const ce = new CommandExchange(
                    commandParsed.commandName, commandParsed.args, this, 'channel', channel, { userstate }, userstate.username
                );
                ce.setUserProvider(this.userProvider);
                this.connectorManager.newCommand(ce);
            } break;
            case 'whisper': {
                const ce = new CommandExchange(
                    commandParsed.commandName, commandParsed.args, this, 'whisper', userstate.username, { userstate }, userstate.username
                );
                ce.setUserProvider(this.userProvider);
                this.connectorManager.newCommand(ce);
            } break;
