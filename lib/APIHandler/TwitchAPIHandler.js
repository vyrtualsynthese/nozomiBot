...
module.exports = class TwitchAPIHandler {
    ...
    async getUserInfoFollow (username) {
        ...
        let channelId = await this.cacheManager.getObject(`channel_info_${this.twitchChannelName}`);
        if (!channelId) {
            channelId = await this.getUserInfoByUsername(this.twitchChannelName);
            this.cacheManager.setObject(`channel_info_${this.twitchChannelName}`, channelId, 3600);
        }

        const userInfoFollow = await helixUser.getFollowTo(channelId._data.id);
        if (!userInfoFollow) {
