...
module.exports = class FollowsCheckCommand extends Command {
    ...
    async handle (commandExchange) {
        ...
        let userInfoFollow = await this.twitchApiHandler.checkFollowCached(username);

        if (!userInfoFollow) {
            let userInfo = await this.twitchApiHandler.getUserInfoByUsername(username);

            if (userInfo === `no`) {
                response = `${username} n'existe pas.`;
                return this._createResponse(commandExchange, response);
            }

            userInfoFollow = await this.twitchApiHandler.getUserInfoFollow(userInfo);
        }
        if (userInfoFollow === `no`) {
            response = `${username} ne follow pas ${process.env.TWITCH_CHANNEL.substring(1)}.`;
            return this._createResponse(commandExchange, response);
        }

        const until = moment(userInfoFollow._data.followDate).format('LLL');
