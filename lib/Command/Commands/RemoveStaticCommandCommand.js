...
module.exports = class RemoveStaticCommandCommand extends Command {
    ...
    async handle (commandExchange) {
        ...
        let response = null;
        await this.staticCommandRepository.create(staticCommand).catch(err => {
            this.logger.error(`failed to remove the custom command ${commandName} : ${err}`);
            response = this._sendResponse(commandExchange, `Impossible de supprimer la commande custom "${commandName}".`);
        });
        if (response !== null) {
            return response;
        }
        this.logger.info(`custom command ${commandName} has been removed`);
        return this._sendResponse(commandExchange, `La commande custom "${commandName}" a été supprimée.`);
    }
