...
module.exports = class RemoveStaticCommandCommand extends Command {
    ...
    async handle (commandExchange) {
        ...
        let staticCommand = await this.staticCommandRepository.findOneByName(commandName).catch(err => {
            this.logger.warning(`failed to find the command ${commandName} : ${err}`);
        });
        ...
        this.staticCommandRepository.remove(staticCommand).then(() => {
            this.logger.info(`static command ${commandName} has been removed`);
            this._sendResponse(commandExchange, `La commande ${commandName} a été supprimée.`);
        }).catch(err => {
            this.logger.error(`failed to remove the command ${commandName} : ${err}`);
            this._sendResponse(commandExchange, `Impossible de supprimer la commande ${commandName}.`);
        });
    }
