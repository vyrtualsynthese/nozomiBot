...
module.exports = class CreateStaticCommandCommand extends Command {
    ...
    async handle (commandExchange) {
        ...
        this.staticCommandRepository.create(newStaticCommand).then(() => {
            ...
            this._sendResponse(commandExchange, `Nouvelle commande custom ajoutée : ${commandName}.`);
        }).catch(err => {
            this.logger.error(`failed to add new custom command : ${err}`);
            this._sendResponse(commandExchange, `Impossible de créer la commande custom ${commandName}.`);
        });
    }
